...
#define MAX_IMAGE_DATA_SIZE (4 * 2048 * 3072)
    ...
    Camera::Camera(ros::NodeHandle &node)
    {
        ...
        node.param("Camera_id", Camera_id, std::to_string('0'));
        node.param("rostopic_name", rostopic_name, std::to_string('/hikrobot_camera'));
        node.param("width", width, 1280);
        node.param("height", height, 1024);
        node.param("FrameRateEnable", FrameRateEnable, false);
        node.param("FrameRate", FrameRate, 90);
        ...
        for (int i = 0; i < stDeviceList.nDeviceNum; i++)
        {
            ...
            {
                MV_USB3_DEVICE_INFO stUsb3VInfo = pDeviceInfo->SpecialInfo.stUsb3VInfo;


                unsigned char* ch_ptr = (unsigned char *)Camera_id.c_str();
                if(*(stUsb3VInfo.chUserDefinedName) == *(ch_ptr))
                {
            ...
            if(i == stDeviceList.nDeviceNum - 1)
            {
                printf("Find No %s Device!\n",Camera_id.c_str());
                exit(-1);
            }
        }
